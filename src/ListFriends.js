...
class ListFriends extends React.Component {
    ...
    componentDidMount() {
        console.log("component did mount for ListFriends");
        !this.users && this.props.dispatch(receiveUsers());
    }

    render() {
        console.log("rendering ListFriends");
        ...
        return (
            <div>
                ...
                {!users.length && <div>Nobody is your friend</div>}
...
export default withStyles(styles)(
    connect(state => {
        return {
            users: state.users && state.users.filter(user => user.status == 2)
        };
    })(ListFriends)
);
